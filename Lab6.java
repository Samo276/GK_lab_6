...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {    
		...
		float[] position2 = { 2,1,4,1 };
		
		gl2.glLightfv(gl2.GL_LIGHT1, gl2.GL_POSITION, position,0);
		gl2.glLightfv(gl2.GL_LIGHT2, gl2.GL_POSITION, position2,0);
		
		
		float blue1 [] = {1f, 1f, 1f, 1};
		float blue2 [] = {0.0f, 0.0f, 0.2f, 1};
		
		gl2.glLightfv (gl2.GL_LIGHT1, gl2.GL_DIFFUSE, blue1,0);
		gl2.glLightfv (gl2.GL_LIGHT2, gl2.GL_AMBIENT, blue2,0);
		//gl2.glLightfv (GL_LIGHT1, GL_AMBIENT, blue3);
		
		float ambientLevel[] = { 1f, 1f, 1f, 1f };
		...
		draw_stozek(gl2,3, -4, -1,-4);
		
		gl2.glMaterialfv (GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[10], 0);
							...
							float translacja_z) 
	{
		...
		{
		 draw_trojkat(promien,gl2,i,0,translacja_x,translacja_y,translacja_z);								//podstawa
		 draw_trojkat(promien,gl2,i, wysokosc_stozka,translacja_x,translacja_y,translacja_z);		//sciany
		}
		gl2.glEnd ();
	}
	private void draw_trojkat(int promien, GL2 gl2, int i, float wysokosc, float x, float y, float z) {
		
		gl2.glVertex3f((float)(promien*Math.cos(2*i*Math.PI /3))+x,0+y, (float)(promien*Math.sin(2*i*Math.PI /3)+z));
		
		gl2.glVertex3f((float)(promien*Math.cos(2*(i+1)*Math.PI /3))+x,0+y, (float)(promien*Math.sin(2*(i+1)*Math.PI /3)+z));
		
		gl2.glVertex3f(0+x,wysokosc+y, 0+z);
